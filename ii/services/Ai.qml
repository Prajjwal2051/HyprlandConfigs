...
Singleton {
    ...
    readonly property string safeApiFormat: models[currentModelId]?.api_format || "openai"
    readonly property var availableToolMap: root.tools[safeApiFormat] || root.tools.openai || {}
    property list<var> availableTools: Object.keys(availableToolMap)
    ...
    property ApiStrategy currentApiStrategy: apiStrategies[safeApiFormat] || apiStrategies.openai
    ...
    Component.onCompleted: {
        if (!models[currentModelId] && modelList.length > 0) {
            currentModelId = modelList[0];
        }
    ...
    function getModel() {
        return models[currentModelId] || models[modelList[0]] || null;
    }
    ...
    function setTool(tool) {
        if (!root.availableToolMap || !(tool in root.availableToolMap)) {
