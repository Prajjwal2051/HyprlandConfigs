...
Singleton {
    ...
    property QtObject glassmorphism
    ...
    property bool glassmorphismEnabled: Config?.options.appearance.glassmorphism?.enable ?? false
    property real backgroundTransparency: {
        if (glassmorphismEnabled) return 0.55;
        return Config?.options.appearance.transparency.enable ?
            (Config?.options.appearance.transparency.automatic ?
                ...
                Config?.options.appearance.transparency.backgroundTransparency) : 0;
    }
    property real contentTransparency: {
        if (glassmorphismEnabled) return 0.70;
    ...
    colors: QtObject {
        ...
        // Borders
        property color colBorder: m3colors.m3outline
        property color colBorderVariant: m3colors.m3outlineVariant
    ...
    sizes: QtObject {
        ...
        property real borderWidthThick: 3
        // Bar
    ...
    glassmorphism: QtObject {
        // Glassmorphism: frosted-glass visual theme.
        // Enable compositor blur (e.g. Hyprland's blur) for the full effect.
        property bool enabled: root.glassmorphismEnabled
        property real blurRadius: Config?.options.appearance.glassmorphism?.blurRadius ?? 20
        property real tintStrength: Config?.options.appearance.glassmorphism?.tintStrength ?? 0.08
        property real borderOpacity: Config?.options.appearance.glassmorphism?.borderOpacity ?? 0.30

        // Semi-transparent tinted surface — use as background for glass panels
        property color glassBase: ColorUtils.transparentize(
            ColorUtils.mix(root.m3colors.m3surfaceContainerLow, root.m3colors.m3primary, tintStrength),
            0.55
        )
        // Slightly elevated glass layer (layer 1 equivalent)
        property color glassLayer1: ColorUtils.transparentize(
            ColorUtils.mix(root.m3colors.m3surfaceContainerLow, root.m3colors.m3primary, tintStrength * 0.6),
            0.50
        )
        // Even more elevated glass layer (layer 2 equivalent)
        property color glassLayer2: ColorUtils.transparentize(
            ColorUtils.mix(root.m3colors.m3surfaceContainer, root.m3colors.m3primary, tintStrength * 0.4),
            0.45
        )
        // Translucent border — subtle luminous rim for glass panels
        property color glassBorder: Qt.rgba(
            root.m3colors.m3onSurface.r,
            root.m3colors.m3onSurface.g,
            root.m3colors.m3onSurface.b,
            borderOpacity
        )
        // Top-edge specular highlight for a extra glassy look
        property color glassHighlight: Qt.rgba(1.0, 1.0, 1.0, 0.12)
        // Scrim for modal glass backdrops
        property color glassScrim: ColorUtils.transparentize(root.m3colors.m3scrim, 0.65)
    }
